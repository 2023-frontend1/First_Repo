...
$addBtn.addEventListener("click", (e) => {
  ...
  if(!$formData1.value.trim() || !$formData2.value.trim()) {
    ...
    $tbody.append($trData);
    $trData.innerHTML +=`
      <td class="item">`+$formData1.value+`</td>
      <td class="weight">`+$formData2.value+`</td>
...
$submitBtn.addEventListener("click", (e) => {
  ...
  for (let element of $hasData) {
    ...
    if(element.hasChildNodes()) {
      for(var i=0; i< element.childNodes.length; i++){                     // tr의 인덱스를 구하고,
        ...
        $result.innerHTML +=`
          <li>` + $listItem.innerHTML + ` : ` + $listWeight.innerHTML + `</li>
        `;                                                                  // 추가해준다
      }
    } else {
      alert("추가할 내용이 없습니다.")
